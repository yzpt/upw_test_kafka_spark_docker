...
git add . && git commit -m "kafka python client ACL ok"
git push --set-upstream origin zookeeper_acl
...
# try with --driver-java-options -Djava.security.auth.login.config=/opt/bitnami/spark/conf/kafka_broker_jaas.conf
